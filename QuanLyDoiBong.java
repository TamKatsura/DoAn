...
public class QuanLyDoiBong {
    ...
    static void docTuFile(String file) {
        try {
            ...
            String line;
            ArrayList<CauThu> cauThuList = new ArrayList<>();
            ...
            themCauThuTuDanhSach(cauThuList);
        } catch (IOException e) {
            System.out.println("Có lỗi khi đọc file: " + e.getMessage());
        }
    }

    static void themCauThuTuDanhSach(ArrayList<CauThu> cauThuList) {
        for (CauThu cauThu : cauThuList) {
            themCauThu(cauThu.maCauThu, cauThu.tenCauThu, cauThu.tuoi, cauThu.viTri, cauThu.quocTich, cauThu.doiBong);
        }
    }

    static void themCauThu(String maCauThu, String tenCauThu, int tuoi, String viTri, String quocTich, String doiBong) {
        if (soLuongCauThu < MAX_CAUTHU) {
            QuanLyDoiBong.maCauThu[soLuongCauThu] = maCauThu;
            QuanLyDoiBong.tenCauThu[soLuongCauThu] = tenCauThu;
            QuanLyDoiBong.tuoiCauThu[soLuongCauThu] = tuoi;
            QuanLyDoiBong.viTri[soLuongCauThu] = viTri;
            QuanLyDoiBong.quocTich[soLuongCauThu] = quocTich;
            QuanLyDoiBong.doiBong[soLuongCauThu] = doiBong;
            soLuongCauThu++;
            System.out.println("Đã thêm cầu thủ mới vào đội bóng!");
        } else {
    ...
    static void xuatRaFile(String file) {
        try {
            FileWriter fw = new FileWriter(file);
            BufferedWriter bw = new BufferedWriter(fw);
            for (int i = 0; i < soLuongCauThu; i++) {
                bw.write(maCauThu[i] + "," + tenCauThu[i] + "," + tuoiCauThu[i] + "," + viTri[i] + "," + quocTich[i]
                        + "," + doiBong[i]);
                bw.newLine();
            }
            bw.close();
